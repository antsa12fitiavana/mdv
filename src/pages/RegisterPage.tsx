...
export default function RegisterPage() {
  ...
  const steps = [t("identity"), t("territory"), t("roleLabel"), t("paymentLabel")];
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4 py-8 relative overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-6 px-4">
          {steps.map((s, i) => (
            <div key={s} className="flex-1 flex items-center gap-2">
              ...
              {i < 3 && <div className={`flex-1 h-0.5 ${i < step ? "bg-amber-500" : "bg-slate-700"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
          ...
          {step === 2 && (
            <div className="space-y-4">
              <Label className="text-xs text-slate-400">{t("roleLabel")}</Label>
            ...
            <div className="space-y-4">
              ...
              <RadioGroup value={form.paymentMethod} onValueChange={(v) => setForm({ ...form, paymentMethod: v })} className="space-y-2">
                <label className="flex items-center gap-3 p-3 rounded-lg border border-slate-700 cursor-pointer hover:border-slate-600">
                  ...
                  <span className="text-sm text-slate-300">Mobile Money (mVola / Orange Money)</span>
                </label>
                <label className="flex items-center gap-3 p-3 rounded-lg border border-slate-700 cursor-pointer hover:border-slate-600">
                  <RadioGroupItem value="card" className="border-slate-500" />
                  <span className="text-sm text-slate-300">Carte bancaire</span>
                </label>
              </RadioGroup>
