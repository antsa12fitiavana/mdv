...
import { Package, TrendingUp, Scissors, ScanLine, HandCoins, Plane, Gem, FileStack } from "lucide-react";

export default function PierreComptoirHome() {
    const { user } = useAuth();
    const { t } = useLanguage();
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-slate-500 mt-1">{t("dashboard")} - {t("comptoir")} {t("filierePierre")}</p>
            </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-slate-900">{myLots.filter(l => l.status === "transformed").length}</p>
                                <p className="text-xs text-slate-500">Lots polis</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <Button variant="outline" onClick={() => navigate("/dashboard/exports")}>
                    <Plane className="w-4 h-4 mr-2" />Exports
                </Button>
            </div>
        </div>
    );
